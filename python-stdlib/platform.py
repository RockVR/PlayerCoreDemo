...
def is_simulator():
    ...
    Only useful when working with iOS, tvOS or visionOS, because
...
def _syscmd_file(target, default=''):
    ...
    """
    if sys.platform in ('dos', 'win32', 'win16', 'ios', 'tvos', 'xros'):
        # XXX Others too ?
        return default
...
class _Processor:
    ...
    # On iOS, tvOS and visionOS, os.uname returns the architecture
...
def uname():
    ...
    if sys.platform in ('ios', 'tvos', 'xros'):
...
def platform(aliased=0, terse=0):
    ...
    if system == 'Darwin':
        # macOS/iOS/tvOS/visionOS (darwin kernel)
        if sys.platform in ('ios', 'tvos', 'xros'):
